...
async function main() {
  ...
  // TX 1: setPrimaryRouter
  transactions.push({
    to: LE,
    value: "0",
    data: le.interface.encodeFunctionData("setPrimaryRouter", [ADAPTER]),
    contractMethod: null,
    contractInputsValues: null,
  });

  // TX 2-15: whitelistToken
  for (const t of newTokens) {
    transactions.push({
      to: TV,
      value: "0",
      data: tv.interface.encodeFunctionData("whitelistToken", [t.address]),
      contractMethod: null,
      contractInputsValues: null,
    });
  }

  // TX 16-29: setPriceFeed
  for (const t of newTokens) {
    transactions.push({
      to: PO,
      value: "0",
      data: po.interface.encodeFunctionData("setPriceFeed", [t.address, ZERO, t.decimals]),
      contractMethod: null,
      contractInputsValues: null,
    });
  }
  ...
  const batch = {
    ...
    createdAt: Date.now(),
    meta: {
      name: "NomoLend: Add 14 new collateral tokens",
      description: "Set multihop adapter as primary router, whitelist 14 tokens, register price feeds",
      txBuilderVersion: "1.16.5",
    },
    transactions,
  };

  const path = "safe-batch-add-tokens.json";
  writeFileSync(path, JSON.stringify(batch, null, 2));
  console.log(`Safe batch JSON written to: ${path}`);
  console.log(`Total transactions: ${transactions.length}`);
  console.log("\nSteps:");
  console.log("1. Go to https://app.safe.global");
  console.log("2. Open your Safe: 0x362D5267A61f65cb4901B163B5D94adbf147DB87");
  console.log("3. New Transaction -> Transaction Builder");
  console.log("4. Upload this JSON file");
  console.log("5. Review and sign");
}
main();
